...
class IMGD_Propiedades {
	...
	private function __construct() {
		...
		add_filter('manage_imgd_propiedad_posts_columns', array($this, 'posts_columns'));
		add_action('manage_imgd_propiedad_posts_custom_column', array($this, 'posts_custom_columns'));
		...
		$meta_boxes[] = array(
			...
			'fields' => array(
				array(
					...
					'desc' => __('Se muestra en la Home Page', 'imgd_framework'),
				...
				array(
					// Field name - Will be used as label
					'name' => __('SlideShow', 'imgd_framework'),
					// Field ID, i.e. the meta key
					'id' => 'imgd_slideshow',
					// Field description (optional)
					'desc' => __('Se muestra en el Slide Show de la Home Page', 'imgd_framework'),
					// CLONES: Add to make the field cloneable (i.e. have multiple value)
					'clone' => false,
					'type' => 'checkbox'
				),
...
// 1st meta box
	...
	public function posts_custom_columns($column_name) {
		...
		if ($column_name === 'precio') {
			$data = rwmb_meta('imgd_propiedad_precio');
		...
		if ($column_name === 'description') {
			//echo data;
			echo imgd_propiedad_datos();
		}

		if ($column_name === 'hom') {
			$meta_desc = rwmb_meta('imgd_home');

			if ($meta_desc === '1')
				echo "<i class='icon-ok-sign'></i>";
		}
		if ($column_name === 'sli') {
			$meta_desc = rwmb_meta('imgd_slideshow');

			if ($meta_desc === '1')
				echo "<i class='icon-ok-sign'></i>";
		}
		if ($column_name === 'des') {
			$meta_desc = rwmb_meta('imgd_propiedad_destacada');
		...
		if ($column_name === 'ciudad') {
			echo the_terms('', 'prop_ciudad');
		}
		if ($column_name === 'tipo') {
			echo the_terms('', 'prop_tipo');
		}
		if ($column_name === 'cod') {
			echo rwmb_meta('imgd_propiedad_codigo');
		}
